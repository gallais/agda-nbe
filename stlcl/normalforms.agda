...
data _⊢whne_ {n} (Γ : Con (ty n)) : ty n → Set where
  `v    : ∀ {τ} (pr : τ ∈ Γ) → Γ ⊢whne τ
...
data _⊢whnf_ {n} (Γ : Con (ty n)) : ty n → Set where
...
back-whne : ∀ {n Γ} {σ : ty n} (t : Γ ⊢whne σ) → Γ ⊢ σ
back-whne (`v pr) = `v pr
...
back-whnf : ∀ {n Γ} {σ : ty n} (t : Γ ⊢whnf σ) → Γ ⊢ σ
...
weaken-whne : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢whne σ) → Δ ⊢whne σ
...
weaken-whnf : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢whnf σ) → Δ ⊢whnf σ
...
back-weaken-whne : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢whne σ) →
  back-whne (weaken-whne inc t) ≡ ⊢-weaken inc (back-whne t)
back-weaken-whne inc (`v pr) = refl
back-weaken-whne inc (f `$ x) rewrite back-weaken-whne inc f = refl
back-weaken-whne inc (`π₁ t) = cong `π₁ (back-weaken-whne inc t)
back-weaken-whne inc (`π₂ t) = cong `π₂ (back-weaken-whne inc t)
back-weaken-whne inc (`map f xs) = cong (`map _) (back-weaken-whne inc xs)
back-weaken-whne inc (`fold c n xs) = cong (`fold _ _) (back-weaken-whne inc xs)
back-weaken-whne inc (xs `++ ys) rewrite back-weaken-whne inc xs = refl

back-weaken-whnf : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢whnf σ) →
  back-whnf (weaken-whnf inc t) ≡ ⊢-weaken inc (back-whnf t)
back-weaken-whnf inc (`↑ t) = back-weaken-whne inc t
back-weaken-whnf inc (`λ t) = refl
back-weaken-whnf inc `⟨⟩ = refl
back-weaken-whnf inc (a `, b) = refl
back-weaken-whnf inc `[] = refl
back-weaken-whnf inc (hd `∷ tl) = refl

infix 5 _⊢ne_ _⊢nf_

mutual

  data _⊢ne_ {n} (Γ : Con (ty n)) (τ : ty n) : Set where
    `v : ∀ (pr : τ ∈ Γ) → Γ ⊢ne τ
  ...
  data _⊢nf_ {n} (Γ : Con (ty n)) : ty n → Set where
    `↑ : ∀ {k} (t : Γ ⊢ne `b k) → Γ ⊢nf `b k
...
mutual
  ...
  back-ne : ∀ {n Γ} {σ : ty n} (t : Γ ⊢ne σ) → Γ ⊢ σ
  back-ne (`v pr) = `v pr
  ...
  back-nf : ∀ {n Γ} {σ : ty n} (t : Γ ⊢nf σ) → Γ ⊢ σ
  back-nf (`↑ t) = back-ne t
...
mutual
  ...
  nf-whnf : ∀ {n Γ} {σ : ty n} (t : Γ ⊢nf σ) → Γ ⊢whnf σ
  nf-whnf (`↑ t) = `↑ (ne-whne t)
  ...
  ne-whne : ∀ {n Γ} {σ : ty n} (t : Γ ⊢ne σ) → Γ ⊢whne σ
  ne-whne (`v pr) = `v pr
...
mutual
  ...
  ne-weaken : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢ne σ) → Δ ⊢ne σ
  ...
  nf-weaken : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢nf σ) → Δ ⊢nf σ
  nf-weaken inc (`↑ t) = `↑ (ne-weaken inc t)
...
mutual
  ...
  ne-weaken-refl : ∀ {n Γ} {σ : ty n} (t : Γ ⊢ne σ) → ne-weaken (same Γ) t ≡ t
  ...
  nf-weaken-refl : ∀ {n Γ} {σ : ty n} (t : Γ ⊢nf σ) → nf-weaken (same Γ) t ≡ t
  nf-weaken-refl (`↑ t) = cong `↑ (ne-weaken-refl t)
...
mutual
  ...
  ne-weaken² : ∀ {n Γ Δ} (inc : Γ ⊆ Δ) {Ε} (inc' : Δ ⊆ Ε) {σ : ty n} (t : Γ ⊢ne σ) →
  ...
  nf-weaken² : ∀ {n Γ Δ} (inc : Γ ⊆ Δ) {Ε} (inc' : Δ ⊆ Ε) {σ : ty n} (t : Γ ⊢nf σ) →
    nf-weaken inc' (nf-weaken inc t) ≡ nf-weaken (⊆-trans inc inc') t
  nf-weaken² inc inc' (`↑ t) = cong `↑ (ne-weaken² inc inc' t)
...
mutual
  ...
  ne-weaken-back : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢ne σ) →
  ...
  nf-weaken-back : ∀ {n Γ Δ} {σ : ty n} (inc : Γ ⊆ Δ) (t : Γ ⊢nf σ) →
                   back-nf (nf-weaken inc t) ≡ ⊢-weaken inc (back-nf t)
  nf-weaken-back inc (`↑ t) = ne-weaken-back inc t
